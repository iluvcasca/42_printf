...
/*   Updated: 2023/12/17 17:06:49 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void lexer_type(char ** format, t_lexer_status * lexer_status, va_list ap)
{
    ...
    if (lexer_status->lexer_state == TYPE/* && !isEOF(*format, lexer_status->lexer_flags.i)*/)
    {
        if (current_char == '%')
            ...
            process_type(format, lexer_status, &((int){(va_arg(ap, int))}), lexer_putchar);
        else if (current_char == 's')
            process_type(format, lexer_status, va_arg(ap, char *), lexer_putstr);
        else if (current_char == 'p')
            process_type(format, lexer_status, va_arg(ap, void *), lexer_pointer);
