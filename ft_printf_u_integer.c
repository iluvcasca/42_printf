...
/*   Updated: 2023/12/24 12:09:47 by kgriset          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	void lexer_u_integer(t_lexer_status *lexer_status, void *arg)
{
    ...
    {
        printf_utoa_size((*((unsigned int *)arg)), &u_int_len); 
        u_int_str = malloc(sizeof(char) * (u_int_len));
        if (!u_int_str)
            return;
        printf_utoa((*((unsigned int *)arg)), lexer_status, u_int_str, u_int_len);
    }
		...
		size_t u_int_len, void *arg)
	{
		...
		prefix = u_integer_get_preffix_size(lexer_status, &u_int_len,*((unsigned int *)arg));
		if (lexer_status->width)
		{
			if (lexer_status->lexer_flags.minus)
        {
				printf_u_integer_write(lexer_status, u_int_str, u_int_len);
                printf_width(lexer_status, lexer_status->width - u_int_len - prefix,
				' ');
        }
			if (!lexer_status->lexer_flags.minus)
        {
